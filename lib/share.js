...
export function buildShareContent(tracker, baseUrl) {
  ...
  const imageUrl = `${baseUrl}/og-image.png?v=${encodeURIComponent(tracker.stateId)}`
...
export async function buildOgImagePng(tracker) {
  const svg = buildOgImageSvg(tracker)
